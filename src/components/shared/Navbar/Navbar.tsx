...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-6 inset-x-4 h-16 max-w-screen-2xl mx-auto rounded-full bg-background border dark:border-slate-700/70 z-30">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          {
            session.status === "authenticated" ? (<Button
              variant="destructive"
              className="w-full justify-start gap-2 cursor-pointer rounded-full px-5 py-2 text-sm md:text-base"
              onClick={() => signOut()}
            >
